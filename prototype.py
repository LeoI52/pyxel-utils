...
# Derived values
NUM_COLS = ZONE_W // CELL_SIZE
NUM_ROWS = ZONE_H // CELL_SIZE

# Each column can have one cell (None if empty)
cells = [None for _ in range(NUM_COLS)]

class App:
    def __init__(self):
        pyxel.init(130, 80, title="Click Cells Example")
        pyxel.mouse(True)
        pyxel.run(self.update, self.draw)

    def update(self):
        # Get mouse position relative to the zone
        mx, my = pyxel.mouse_x, pyxel.mouse_y
        if (ZONE_X <= mx < ZONE_X + ZONE_W) and (ZONE_Y <= my < ZONE_Y + ZONE_H):
            col = (mx - ZONE_X) // CELL_SIZE
            row = (my - ZONE_Y) // CELL_SIZE

            if pyxel.btn(pyxel.MOUSE_BUTTON_LEFT):
                cells[col] = row   # set new cell
            if pyxel.btn(pyxel.KEY_SPACE):
                cells[col] = None

        # Press space to clear all cells
    ...
    def draw(self):
        pyxel.cls(0)
        # Draw zone border
        pyxel.rectb(ZONE_X, ZONE_Y, ZONE_W, ZONE_H, 7)

        # Draw grid
        for c in range(1, NUM_COLS):
            x = ZONE_X + c * CELL_SIZE
            pyxel.line(x, ZONE_Y, x, ZONE_Y + ZONE_H, 1)
        for r in range(1, NUM_ROWS):
            y = ZONE_Y + r * CELL_SIZE
            pyxel.line(ZONE_X, y, ZONE_X + ZONE_W, y, 1)

        # Draw cells
        for col, row in enumerate(cells):
            if row is not None:
                cell_x = ZONE_X + col * CELL_SIZE
                cell_y = ZONE_Y + row * CELL_SIZE
                pyxel.rect(cell_x + 1, cell_y + 1, CELL_SIZE - 2, CELL_SIZE - 2, 9)
